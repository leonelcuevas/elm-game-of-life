...
var author$project$Grid$rangeNonInc = F2(
	function (start, end) {
		return A2(elm$core$List$range, start, end - 1);
	});
...
var author$project$Grid$coordTuples = F2(
		...
		return A2(
			...
			function (y) {
				return A2(
					...
					A2(author$project$Grid$rangeNonInc, 0, width));
			},
			A2(author$project$Grid$rangeNonInc, 0, height));
	});
...
var author$project$Grid$emptyGrid = F3(
	function (width, height, _default) {
		var coordinates = A2(author$project$Grid$coordTuples, width, height);
		var asocList = A2(
			elm$core$List$map,
			function (c) {
				return _Utils_Tuple2(c, _default);
			},
			coordinates);
		return {
			data: elm$core$Dict$fromList(asocList),
			height: height,
			width: width
		};
	});
var author$project$GameOfLife$createBoard = F2(
	function (width, height) {
		return A3(author$project$Grid$emptyGrid, width, height, author$project$GameOfLife$Dead);
	});
...
var author$project$Main$initialModel = function (flags) {
	var model = {
		autoIterating: false,
		board: A2(author$project$GameOfLife$createBoard, 100, 50),
		canvasHeight: 500,
		canvasWidth: 1000,
		iterationTime: 100,
		mouseMoving: false,
...
var author$project$Main$NextIteration = {$: 'NextIteration'};
...
var author$project$Main$subscriptions = function (model) {
	return model.autoIterating ? A2(
		elm$time$Time$every,
		model.iterationTime,
		function (time) {
			return author$project$Main$NextIteration;
		}) : elm$core$Platform$Sub$none;
};
var author$project$GameOfLife$Alive = {$: 'Alive'};
var author$project$Grid$getNeighborCoords = function (_n0) {
	var x = _n0.a;
	var y = _n0.b;
	return _List_fromArray(
		[
...
var author$project$Grid$getValue = F2(
	function (_n0, coord) {
		var data = _n0.data;
		return A2(elm$core$Dict$get, coord, data);
	});
var elm$core$Basics$ge = _Utils_ge;
var author$project$Grid$isInsideGrid = F2(
	function (grid, _n0) {
		var x = _n0.a;
		var y = _n0.b;
		var w = grid.width;
		var h = grid.height;
		return (x >= 0) && ((_Utils_cmp(x, w) < 0) && ((y >= 0) && (_Utils_cmp(y, h) < 0)));
	});
...
var author$project$Grid$getNeighborValues = F2(
	function (grid, coord) {
		if (A2(author$project$Grid$isInsideGrid, grid, coord)) {
			var neighborCoords = author$project$Grid$getNeighborCoords(coord);
			var maybeNeighbors = A2(
				elm$core$List$map,
				function (c) {
					return A2(author$project$Grid$getValue, grid, c);
				},
				neighborCoords);
			return elm$core$Maybe$Just(
				elm_community$maybe_extra$Maybe$Extra$values(maybeNeighbors));
		} else {
			return elm$core$Maybe$Nothing;
		}
	});
...
var author$project$GameOfLife$nextCellState = F3(
	function (board, coord, cell) {
		var neighbors = A2(author$project$Grid$getNeighborValues, board, coord);
		var aliveNeighbors = function () {
			if (neighbors.$ === 'Just') {
				var neighborList = neighbors.a;
				return A2(
					elm_community$list_extra$List$Extra$count,
					function (el) {
						return _Utils_eq(el, author$project$GameOfLife$Alive);
					},
					neighborList);
			} else {
				return 0;
		...
		if (cell.$ === 'Alive') {
			return ((aliveNeighbors === 2) || (aliveNeighbors === 3)) ? author$project$GameOfLife$Alive : author$project$GameOfLife$Dead;
		} else {
			return (aliveNeighbors === 3) ? author$project$GameOfLife$Alive : author$project$GameOfLife$Dead;
		}
	});
...
var author$project$GameOfLife$nextState = function (board) {
	return _Utils_update(
		board,
		{
			data: A2(
				elm$core$Dict$map,
				author$project$GameOfLife$nextCellState(board),
				board.data)
		});
};
var author$project$Grid$setValue = F3(
	function (grid, coord, val) {
		if (A2(author$project$Grid$isInsideGrid, grid, coord)) {
			var newData = A3(elm$core$Dict$insert, coord, val, grid.data);
			return _Utils_update(
				grid,
				{data: newData});
		} else {
...
var author$project$GameOfLife$setCell = F2(
	function (board, coord) {
		return A3(author$project$Grid$setValue, board, coord, author$project$GameOfLife$Alive);
	});
var author$project$Main$setCellFromMouseCoords = F2(
	function (_n0, _n1) {
		var board = _n0.board;
		var tileSize = _n0.tileSize;
		var p_x = _n1.a;
		var p_y = _n1.b;
		var y = elm$core$Basics$floor(p_y / tileSize);
		var x = elm$core$Basics$floor(p_x / tileSize);
		return A2(
			author$project$GameOfLife$setCell,
			board,
			_Utils_Tuple2(x, y));
	});
			...
			case 'MouseDownAt':
				...
				return _Utils_Tuple2(
					_Utils_update(
						model,
						{
							board: A2(author$project$Main$setCellFromMouseCoords, model, coords),
							mouseMoving: true
						}),
					elm$core$Platform$Cmd$none);
			case 'MouseMoveAt':
				var coords = msg.a;
				var newModel = model.mouseMoving ? _Utils_update(
					model,
					{
						board: A2(author$project$Main$setCellFromMouseCoords, model, coords)
					}) : model;
			...
			case 'NextIteration':
				return _Utils_Tuple2(
					_Utils_update(
						model,
						{
							board: author$project$GameOfLife$nextState(model.board)
						}),
					elm$core$Platform$Cmd$none);
			default:
				return _Utils_Tuple2(
					_Utils_update(
						model,
						{autoIterating: !model.autoIterating}),
					elm$core$Platform$Cmd$none);
		}
	});
var author$project$Main$ToggleAuto = {$: 'ToggleAuto'};
var author$project$Main$MouseDownAt = function (a) {
	return {$: 'MouseDownAt', a: a};
};
var author$project$Main$MouseMoveAt = function (a) {
	return {$: 'MouseMoveAt', a: a};
};
var author$project$Main$MouseUpAt = function (a) {
	return {$: 'MouseUpAt', a: a};
};
var author$project$Grid$toList = F2(
	function (f, grid) {
		return A2(
			elm$core$List$map,
			f,
			elm$core$Dict$toList(grid.data));
	});
var author$project$GameOfLife$toList = author$project$Grid$toList;
var author$project$Main$getDrawingCoords = function (_n0) {
	var board = _n0.board;
	var tileSize = _n0.tileSize;
	return A2(
		author$project$GameOfLife$toList,
		function (_n1) {
			var _n2 = _n1.a;
			var x = _n2.a;
			var y = _n2.b;
			var cell = _n1.b;
			var coord_y = tileSize * y;
			var coord_x = tileSize * x;
			return _Utils_Tuple3(coord_x, coord_y, cell);
		},
		board);
};
...
var author$project$Main$renderSquare = F3(
	function (size, _n0, cmds) {
		var x = _n0.a;
		var y = _n0.b;
		var cell = _n0.c;
		var color = function () {
			if (cell.$ === 'Alive') {
				return A3(joakin$elm_canvas$CanvasColor$rgb, 0, 200, 0);
			} else {
				return A3(joakin$elm_canvas$CanvasColor$rgb, 128, 128, 128);
			}
		}();
		return A5(
			...
			size,
			size,
			A2(joakin$elm_canvas$Canvas$fillStyle, color, cmds));
	});
var author$project$Main$renderBoardHelper = F3(
	function (size, drawingCoords, canvas) {
		return A3(
			elm$core$List$foldl,
			author$project$Main$renderSquare(size),
			canvas,
			drawingCoords);
	});
...
var author$project$Main$renderBoard = function (model) {
	var drawingCoords = author$project$Main$getDrawingCoords(model);
	return A4(
		...
		_List_fromArray(
			[
				...
				function (event) {
					return author$project$Main$MouseDownAt(event.offsetPos);
				}),
				mpizenberg$elm_pointer_events$Html$Events$Extra$Mouse$onMove(
				function (event) {
					return author$project$Main$MouseMoveAt(event.offsetPos);
				}),
				mpizenberg$elm_pointer_events$Html$Events$Extra$Mouse$onUp(
				function (event) {
					return author$project$Main$MouseUpAt(event.offsetPos);
				})
			]),
		A3(author$project$Main$renderBoardHelper, model.tileSize, drawingCoords, joakin$elm_canvas$Canvas$empty));
};
	...
	return A2(
		...
		_List_fromArray(
			[
				...
				A2(elm$html$Html$Attributes$style, 'fontFamily', 'helvetica')
			]),
		_List_fromArray(
			[
				...
				_List_fromArray(
					[
						...
						_List_fromArray(
							[
								elm$html$Html$Events$onClick(author$project$Main$NextIteration)
							]),
						_List_fromArray(
							[
								elm$html$Html$text('Next Iteration')
							])),
						A2(
						elm$html$Html$button,
						_List_fromArray(
							[
								elm$html$Html$Events$onClick(author$project$Main$ToggleAuto)
							]),
						_List_fromArray(
							[
								elm$html$Html$text('Play / Stop')
							]))
					]))
			]));
};
